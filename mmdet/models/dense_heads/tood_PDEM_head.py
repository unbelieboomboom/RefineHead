...
class TOOD_rh_Head(ATSSHead):
    ...
    def __init__(self,
                 ...
                 initial_loss_cls=dict(
                     ...
                     alpha=0.25,
                     loss_weight=1.0),
                 loss_bbox_rh=dict(type='DistributionFocalLoss', loss_weight=0.1),
                 **kwargs):
        ...
        self.rh_stacked_convs = rh_stacked_convs
        self.RH_ReLU = RH_ReLU
        super(TOOD_rh_Head, self).__init__(num_classes, in_channels, **kwargs)
    ...
    def _init_layers(self):
        ...
        self.rh_inter_convs = nn.ModuleList()
        ...
        for i in range(self.rh_stacked_convs):
            if i < self.num_dcn:
                conv_cfg = dict(type='DCNv2', deform_groups=4)
            else:
                conv_cfg = self.conv_cfg
            chn = self.feat_channels + self.num_base_priors * 4 if i == 0 else self.feat_channels
            self.rh_inter_convs.append(
                ConvModule(
                    chn,
                    self.feat_channels,
                    3,
        ...
        self.rh_reg_decomp = TaskDecomposition(self.feat_channels,
                                               self.rh_stacked_convs,
                                               self.rh_stacked_convs * 8,
                                               self.conv_cfg, self.norm_cfg)
        ...
        self.rh_reg_offset_module = nn.Sequential(
            nn.Conv2d(self.feat_channels * self.stacked_convs,
                      self.feat_channels // 4, 1), nn.ReLU(inplace=True),
            nn.Conv2d(self.feat_channels // 4, 4 * 2, 3, padding=1))
        ...
        if self.RH_ReLU is True:
            self.RH_relu = nn.ReLU(inplace=True)
        self.RH_scales = nn.ModuleList(
            [Scale(1.0 / float(self.RH_coeff)) for _ in self.prior_generator.strides])
    ...
    def init_weights(self):
        ...
        for m in self.reg_offset_module:
            if isinstance(m, nn.Conv2d):
                normal_init(m, std=0.001)
        for m in self.rh_reg_offset_module:
            if isinstance(m, nn.Conv2d):
                normal_init(m, std=0.001)
    ...
    def forward(self, feats):
        ...
        bbox_preds = []
        bbox_preds_rh = []
        for idx, (x, scale, RH_scales, stride) in enumerate(
                zip(feats, self.scales, self.RH_scales, self.prior_generator.strides)):
            b, c, h, w = x.shape
            ...
            if self.RH_ReLU is True:
                bbox_pred_detach = self.RH_relu(bbox_pred_detach)
                bbox_pred_detach = RH_scales(bbox_pred_detach)
            else:
                bbox_pred_detach = bbox_pred_detach / self.RH_coeff
            # rh_refined = torch.cat([feat, bbox_pred_detach], dim=1)
            rh_refined = torch.cat([reg_feat, bbox_pred_detach], dim=1)
            rh_inter_feats = []
            for inter_conv in self.rh_inter_convs:
                rh_refined = inter_conv(rh_refined)
                rh_inter_feats.append(rh_refined)
            feat = torch.cat(rh_inter_feats, 1)

            # task decomposition
            avg_feat = F.adaptive_avg_pool2d(feat, (1, 1))
            reg_feat = self.rh_reg_decomp(feat, avg_feat)

            # reg prediction and alignment
            if self.anchor_type == 'anchor_free':
                reg_dist = scale(self.rh_tood_reg(reg_feat).exp()).float()
                reg_dist = reg_dist.permute(0, 2, 3, 1).reshape(-1, 4)
                reg_bbox = distance2bbox(
                    self.anchor_center(anchor) / stride[0],
                    reg_dist).reshape(b, h, w, 4).permute(0, 3, 1,
                ...
                reg_dist = scale(self.rh_tood_reg(reg_feat)).float()
                reg_dist = reg_dist.permute(0, 2, 3, 1).reshape(-1, 4)
                reg_bbox = self.bbox_coder.decode(anchor, reg_dist).reshape(
                    b, h, w, 4).permute(0, 3, 1, 2) / stride[0]
            else:
                raise NotImplementedError(
                    f'Unknown anchor type: {self.anchor_type}.'
                    f'Please use `anchor_free` or `anchor_based`.')
            reg_offset = self.rh_reg_offset_module(feat)
            bbox_pred_rh = self.deform_sampling(reg_bbox.contiguous(),
                                             reg_offset.contiguous())

            invalid_bbox_idx = (bbox_pred_rh[:, [0]] > bbox_pred_rh[:, [2]]) | \
                               (bbox_pred_rh[:, [1]] > bbox_pred_rh[:, [3]])
            invalid_bbox_idx = invalid_bbox_idx.expand_as(bbox_pred_rh)
            bbox_pred_rh = torch.where(invalid_bbox_idx, reg_bbox, bbox_pred_rh)

            bbox_preds_rh.append(bbox_pred_rh)

            # refine
        # return tuple(cls_scores), tuple(bbox_preds)
        if self.tood_cls.training:
            return tuple(cls_scores), tuple(bbox_preds), tuple(bbox_preds_rh)
        else:
            return tuple(cls_scores), tuple(bbox_preds_rh)
    ...
    def loss_single(self, anchors, cls_score, bbox_pred, bbox_pred_RH, labels, label_weights,
                    bbox_targets, alignment_metrics, anchor_list_rh, labels_list_rh, label_weights_list_rh,
                    bbox_targets_list_rh,alignment_metrics_list_rh, stride):
        ...
        anchor_rh = anchor_list_rh.reshape(-1, 4)
        cls_score = cls_score.permute(0, 2, 3, 1).reshape(
            -1, self.cls_out_channels).contiguous()
        bbox_pred = bbox_pred.permute(0, 2, 3, 1).reshape(-1, 4)
        bbox_pred_RH = bbox_pred_RH.permute(0, 2, 3, 1).reshape(-1, 4)
        bbox_targets = bbox_targets.reshape(-1, 4)
        bbox_targets_rh = bbox_targets_list_rh.reshape(-1, 4)
        labels = labels.reshape(-1)
        labels_rh = labels_list_rh.reshape(-1)
        alignment_metrics = alignment_metrics.reshape(-1)
        alignment_metrics_rh = alignment_metrics_list_rh.reshape(-1)
        label_weights = label_weights.reshape(-1)
        label_weights_rh = label_weights_list_rh.reshape(-1)
        targets = labels if self.epoch < self.initial_epoch else (
            labels, alignment_metrics)
        targets_rh = labels_rh if self.epoch < self.initial_epoch else (
            labels_rh, alignment_metrics_rh)
        ...
        loss_cls = cls_loss_func(
            cls_score, targets_rh, label_weights_rh, avg_factor=1.0)

        # FG cat_id: [0, num_classes -1], BG cat_id: num_classes
        bg_class_ind = self.num_classes
        pos_inds = ((labels >= 0)
                    & (labels < bg_class_ind)).nonzero().squeeze(1)
        pos_inds_rh = ((labels_rh >= 0)
                    & (labels_rh < bg_class_ind)).nonzero().squeeze(1)

        if len(pos_inds) > 0:
            pos_bbox_targets = bbox_targets[pos_inds]
            pos_bbox_targets_rh = bbox_targets_rh[pos_inds_rh]
            pos_bbox_pred = bbox_pred[pos_inds]
            pos_bbox_pred_rh = bbox_pred_RH[pos_inds_rh]
            pos_anchors = anchors[pos_inds]
            pos_anchors_rh = anchor_rh[pos_inds_rh]

            pos_decode_bbox_pred = pos_bbox_pred
            pos_decode_bbox_pred_rh = pos_bbox_pred_rh
            pos_decode_bbox_targets = pos_bbox_targets / stride[0]
            pos_decode_bbox_targets_rh = pos_bbox_targets_rh / stride[0]
            ...
            pos_bbox_weight = self.centerness_target(
                ...
                pos_inds]
            pos_bbox_weight_rh = self.centerness_target(
                pos_anchors_rh, pos_bbox_targets_rh
            ) if self.epoch < self.initial_epoch else alignment_metrics_rh[
                pos_inds_rh]
            ...
            loss_bbox_rh = self.loss_bbox_rh(
                ...
                pos_decode_bbox_targets_rh,
                weight=pos_bbox_weight_rh,
                avg_factor=1.0)
        else:
            loss_bbox = bbox_pred.sum() * 0
            loss_bbox_rh = bbox_pred_RH.sum() * 0
            pos_bbox_weight = bbox_targets.new_tensor(0.)
            pos_bbox_weight_rh = bbox_targets_rh.new_tensor(0.)

        return loss_cls, loss_bbox, loss_bbox_rh, alignment_metrics_rh.sum(), pos_bbox_weight.sum(),\
               pos_bbox_weight_rh.sum()

    @force_fp32(apply_to=('cls_scores', 'bbox_preds', 'bbox_preds_rh'))
    def loss(self,
             cls_scores,
             bbox_preds,
             bbox_preds_rh,
        ...
        flatten_bbox_preds_rh = torch.cat([
            bbox_pred_rh.permute(0, 2, 3, 1).reshape(num_imgs, -1, 4) * stride[0]
            for bbox_pred_rh, stride in zip(bbox_preds_rh,
                                         self.prior_generator.strides)
        ], 1)
        ...
        anchor_list_rh, valid_flag_list_rh = self.get_anchors(
            featmap_sizes, img_metas, device=device)
        cls_reg_targets_rh = self.get_targets(
            flatten_cls_scores,
            flatten_bbox_preds_rh,
            anchor_list_rh,
            valid_flag_list_rh,
            gt_bboxes,
            img_metas,
            ...
            gt_labels_list=gt_labels,
            label_channels=label_channels)
        # (anchor_list_rh, labels_list_rh, label_weights_list_rh, bbox_targets_list_rh,
        #  alignment_metrics_list_rh) = cls_reg_targets_rh
        (anchor_list, labels_list, label_weights_list, bbox_targets_list,
         alignment_metrics_list) = cls_reg_targets
        (anchor_list_rh, labels_list_rh, label_weights_list_rh, bbox_targets_list_rh,
         alignment_metrics_list_rh) = cls_reg_targets_rh

        losses_cls, losses_bbox, loss_bbox_rh,\
            cls_avg_factors, bbox_avg_factors, bbox_avg_factors_rh = multi_apply(
                self.loss_single,
                anchor_list,
                cls_scores,
                bbox_preds,
                ...
                anchor_list_rh,
                labels_list_rh,
                label_weights_list_rh,
                bbox_targets_list_rh,
                alignment_metrics_list_rh,
                self.prior_generator.strides)
        ...
        bbox_avg_factor_rh = reduce_mean(
            sum(bbox_avg_factors_rh)).clamp_(min=1).item()
        losses_bbox = list(map(lambda x: x / bbox_avg_factor, losses_bbox))
        loss_bbox_rh = list(map(lambda x: x / bbox_avg_factor_rh, loss_bbox_rh))
        return dict(loss_cls=losses_cls, loss_bbox=losses_bbox, loss_bbox_rh=loss_bbox_rh)
