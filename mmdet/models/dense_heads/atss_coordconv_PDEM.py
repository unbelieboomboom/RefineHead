...
class ATSSHead_coord_RH(AnchorHead):
    ...
    def __init__(self,
        ...
        super(ATSSHead_coord_RH, self).__init__(
            num_classes,
            in_channels,
        ...
        for i in range(self.rh_stacked_convs):
            ...
            self.reg_convs.append(
                ConvModule(
                    ...
                    conv_cfg=self.conv_cfg,
                    norm_cfg=self.norm_cfg))
            chn = self.in_channels + self.num_base_priors * 4 + 2 if i == 0 else self.feat_channels
    ...
    def forward_single(self, x, scale, RH_scales):
        ...
        coord_feat = generate_coordinate(reg_feat.size(),
                                         reg_feat.device)
        rh_refined = torch.cat([reg_feat, bbox_pred_detach, coord_feat], dim=1)
