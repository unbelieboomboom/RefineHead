...
# matplotlib.use('TkAgg')   # use to show the plt window
...
# remove the process of drawing labels
def imshow_det_bboxes(img,
    ...
    if bboxes is not None:
        ...
        bbox_palette = palette_val(get_palette(bbox_color, 8+1))
        colors = [bbox_palette[colorkind]]
...
def imshow_gt_det_bboxes(img,
                         ...
                         sup_bbox_color=(61, 241, 102),
                         sup_text_color=(200, 200, 200),
                         sup_mask_color=(61, 241, 102),
    ...
    if isinstance(sup_bboxes, torch.Tensor):
        sup_bboxes = sup_bboxes.detach().cpu().numpy()
    ...
    if overlay_gt_pred:
        ...
        img = imshow_det_bboxes(
            ...
            segms=None,
            # class_names=class_names,
            bbox_color=sup_bbox_color,
            text_color=sup_text_color,
            mask_color=sup_mask_color,
            thickness=thickness,
            font_size=font_size,
            win_name=win_name,
            show=True)
...
def save_bboxes(gt_bboxes, bboxes, sup_bboxes, img_name, img_path, model="", save_dir="./"):
    '''
        save the four coordinates of bboxes (gt, prediction, suppressed) to a json;
        the name represent the name of the model
    '''
    ...
    for box in gt_bboxes:
        save_json["gt_bboxes"].append(box.tolist())
    for box in bboxes:
        save_json["bboxes"].append(box.tolist())
    for box in sup_bboxes:
        save_json["sup_bboxes"].append(box.tolist())

    json_name = os.path.basename(model)
    json_name = os.path.splitext(json_name)[0]
    png_name = os.path.splitext(os.path.basename(img_name))[0]
    save_dir = os.path.join(save_dir, png_name)
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    with open('{}/{}.json'.format(save_dir, json_name), 'w') as output_json_file:
        json.dump(save_json, output_json_file)
