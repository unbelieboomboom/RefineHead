...
# dirtree.remove_node('GFL', 1, reverse=True)
...
def match_gt_pd(best_pd, pd_woNMS, gt, imgIdx, COCOdataset):
    ...
    for counts, (image_id, v) in enumerate(gt.items()):
        # print("idx=" + str(counts) +",best_pd_count="+str(best_pd_count))
        # imgInf = imgIdx.get(ii)
        idkey = COCOdataset.img_ids.index(image_id)
        # image_id = imgInf.get("id")
        gt_list = gt[image_id]
        # image_id = gt_list[0]['image_id']
        # best_pd_list = best_pd[image_id]
        # pd_woNMS_list = pd_woNMS[image_id]
        # idkey = COCOdataset.img_ids.index(image_id)
...
def main():
    ...
    file = open('Cityscapesexperiments/test.txt', 'w')
    ...
    for i in range(len(with_nms_path)):
        ...
        best_pd = read_predictions(with_nms_path[i])
        woNMS_pd = read_predictions(without_nms_path[i])
        # label = os.path.basename(labels[i])
        labeli = os.path.basename(labels[i])
        label = labels[i]

        match_results, best_pd_count, best_count, sup_count = match_gt_pd(best_pd, woNMS_pd, gtAnns, imgIdx, COCOdataset)
